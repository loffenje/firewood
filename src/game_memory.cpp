...
inline u8 alignAdjustment(void *address, u8 alignment) {
    u8 adjustment =
	alignment - (reinterpret_cast<uintptr_t>(address) & (alignment - 1));
...
struct Allocator {
	...
	: size{size}, start{start}, used_memory{0}, num_allocations{0} {}
...
LinearAllocator::LinearAllocator(size_t size, void *start)
    : Allocator(size, start), current_address{start} {
    assert(size > 0);
}
...
void *StackAllocator::allocate(size_t size_in_bytes, u8 alignment) {
    ...
    u8 adjustment =
	memory::alignAdjustmentWithHdr(current_address, alignment, sizeof(Hdr));
    if (used_memory + adjustment + size_in_bytes > size) return nullptr;

    void *aligned_address = memory::add(current_address, adjustment);

    Hdr *header =
	reinterpret_cast<Hdr *>(memory::sub(aligned_address, sizeof(Hdr)));
...
void StackAllocator::deallocate(void *ptr) {
    ...
    used_memory -= reinterpret_cast<uintptr_t>(current_address) -
		   reinterpret_cast<uintptr_t>(ptr) + header->adjustment;
