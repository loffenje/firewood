...
internal void SDLx_ProcessEvents(SDLx_State &state,
	...
	switch (event.type) {
		...
		if (event.key.repeat == 0) {
			...
			SDLx_ProcessKeyboardEvent(&keyboard->move_left,
						  is_down);
		    } else if (key_code == SDLK_s) {
			SDLx_ProcessKeyboardEvent(&keyboard->move_down,
						  is_down);
		    } else if (key_code == SDLK_d) {
			SDLx_ProcessKeyboardEvent(&keyboard->move_right,
						  is_down);
		    } else if (key_code == SDLK_q) {
			SDLx_ProcessKeyboardEvent(&keyboard->left_shoulder,
						  is_down);
		    } else if (key_code == SDLK_e) {
			SDLx_ProcessKeyboardEvent(&keyboard->right_shoulder,
						  is_down);
		    } else if (key_code == SDLK_UP || key_code == SDLK_h) {
			SDLx_ProcessKeyboardEvent(&keyboard->action_up,
						  is_down);
		    } else if (key_code == SDLK_LEFT) {
			SDLx_ProcessKeyboardEvent(&keyboard->action_left,
						  is_down);
		    } else if (key_code == SDLK_DOWN || key_code == SDLK_j) {
			SDLx_ProcessKeyboardEvent(&keyboard->action_down,
						  is_down);
		    } else if (key_code == SDLK_RIGHT) {
			SDLx_ProcessKeyboardEvent(&keyboard->action_right,
						  is_down);
			...
			SDLx_ProcessKeyboardEvent(&keyboard->start, is_down);
			       ...
			       (key_code == SDLK_LSHIFT) ||
			       (key_code == SDLK_RSHIFT)) {
			b32 either_down =
			    (SDL_GetKeyboardState(0)[SDL_SCANCODE_SPACE] ||
...
internal void SDLx_OpenGameControllers() {
	...
	rumble_handles[controller_index] =
...
internal f32 SDLx_GetSecondsElapsed(u64 start, u64 end) {
    f32 result =
	static_cast<f32>(end - start) / static_cast<f32>(g_perf_counter);
...
internal void initializeGameSystems(GameRoot &game_root, SDLx_State &state) {
    ...
    void *game_memory_block = mmap(0, state.total_size, PROT_READ | PROT_WRITE,
    ...
    auto game_partition =
	new LinearAllocator(MB(16), memory::add(game_memory_block, MB(16)));
...
int main() {
    ...
    state.buildEXEFileName("game.so", sizeof(src_game_dll_fullpath),
			   src_game_dll_fullpath);
    SDL_Window *window = SDL_CreateWindow(
	"stuff", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 960, 540,
	...
	*new_keyboard_controller = {};
	...
	for (int button_index = 0;
	     button_index < ARRAY_LEN(new_keyboard_controller->buttons);
	     ++button_index) {
	    new_keyboard_controller->buttons[button_index].ended_down =
		old_keyboard_controller->buttons[button_index].ended_down;
	}

	new_keyboard_controller->clutch_max =
	    old_keyboard_controller->clutch_max;
    ...
    OpenGL *context =
	reinterpret_cast<OpenGL *>(game_root.renderer_api->getContext());
