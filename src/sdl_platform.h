...
void SDLx_State::buildEXEFileName(const char *name, size_t dest_count,
				  char *dest) {
    concatStr(past_last_exepath_slash - exepath, exepath, strlen(name),
	      reinterpret_cast<char *>(const_cast<char *>(name)), dest_count,
	      dest);
}
